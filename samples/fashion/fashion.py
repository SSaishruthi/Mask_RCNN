...
class FashionConfig(Config):
    ...
    NUM_CLASSES = 1 + 1  # Background + fashion
...
class FashionDataset(utils.Dataset):
    ...
    def load_fashion(self, dataset_dir, subset):
       ...
       #
        class_file = scipy.io.loadmat(dataset_dir + 'label_list.mat')
        labels = class_file['label_list']
       #
        label_list = [ ]
        for i in range(len(labels[0])):
              label_list.append(labels[0][i][0])
              
        # Add classes. We have 59 classes.
        for i in range(len(label_list)):
              self.add_class("fashion", i, label_list[i])
       #
        files = glob.glob(dataset_dir+"input/*")
